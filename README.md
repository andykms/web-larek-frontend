...
Рендерит карточку, вставляется на главной странице. В нем проставляется необходимая информация о товаре - категория, название, цена и изображение.

**Класс openProduct**

Наследуется от базового представления ```View```
Рендерит карточку при клике на нее на главной странице, затем вставляется в модальное окно. В нем проставляется более подробная информация о товаре - категория, название, цена, изображение и описание.
Также в представлении есть кнопка для добавления товара в корзину.

**Класс basketProduct**

Наследуется от базового представления ```View```.
Рендерит карточку товара, которая вставлется в представление корзины. 

В нем проставляется краткая информация о товаре - название, цена. Также в представлении есть кнопка для удаления товара из корзины.

**Класс Basket**

Наследуется от базового представления ```View```
Рендерит контент корзины, который вставляется в модальное окно. 
...
Представление использует структуру данных,  хэш-таблица - объект ```Map``` для хранения карточек и последующего вывода их, что ускоряет время работы приложения.

**Класс ContactForm**

Наследуется от базового представления ```Form```
Рендерит контент формы с полями E-mail и телефона.
Вставляется в представление модального окна после этапа заполнения адреса и способа получения заказа.
В представлении есть кнопка для отправки формы .(!!!само представление не отправляет форму!!!)
